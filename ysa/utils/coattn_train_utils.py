...
def train_loop_classification_coattn(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    # tqdm 적용: 진행 표시줄 생성
    pbar = tqdm(enumerate(loader), total=len(loader), desc=f"Epoch {epoch} [Train]", unit="batch")
    ...
    print(f'\rEpoch: {epoch}, train_loss: {train_loss:.4f}, train_acc: {acc:.4f}, train_auc: {auc:.4f}')
...
def validate_classification_coattn(cur, epoch, model, loader, n_classes, early_stopping=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    pbar = tqdm(enumerate(loader), total=len(loader), desc=f"Epoch {epoch} [Val]", unit="batch", leave=False)
...
def summary_classification_coattn(model, loader, n_classes):
    ...
    pbar = tqdm(enumerate(loader), total=len(loader), desc="[Final Eval]", unit="batch", leave=False)
