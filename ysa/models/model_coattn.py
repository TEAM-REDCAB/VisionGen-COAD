...
### MCAT Implementation ###
###########################
class MCAT_Classifier(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=2,
        ...
        super(MCAT_Classifier, self).__init__()
        self.fusion = fusion
    ...
    def forward(self, **kwargs):
        ...
        Y_prob = F.softmax(logits, dim=1)          # [1, n_classes] 클래스별 확률
        Y_hat = torch.topk(logits, 1, dim=1)[1]   # [1, 1] 예측 클래스 (0=MSS, 1=MSI-H)
    ...
    def captum(self, x_path, x_omic1, x_omic2, x_omic3, x_omic4, x_omic5, x_omic6):
        ...
        Y_prob = F.softmax(logits, dim=1)  # [B, n_classes] 클래스별 확률
        return Y_prob
