# 최대 에폭 늘리고 Early Stopping 빡세게 => main_teacher_v2_1
...
4. Hold-out Final Test Set 분리 전략 (다이어그램 기준)
...
5. [v2_1] 방법 B: AUC 1.0 과다 방지 Early Stopping 강화
   - max_epochs: 20 → 100  (충분한 에폭 확보로 ES가 실제로 작동하도록)
   - es_patience: 10 → 7   (더 빡빡한 조기 종료)
   - es_stop_epoch: 20 → 15 (더 이른 감시 시작)
   - stopped_epoch 콘솔/CSV 출력 (main_val_v5_4 방식 그대로 이식)

실행:
    python main_teacher_v2_1.py
"""
...
PATHS = {
    ...
    'wsi_dir':  '/home/team1/data/trident_processed/20.0x_256px_0px_overlap/features_uni_v2',
    'mcat':     './mcat',
    'save_dir': './results_teacher_v2_1',
}
...
CFG = {
    'n_folds':       5,
    'max_epochs':    50,   # 💡 [v2_1] 20 → 50: ES가 실제로 작동하도록 여유 확보
    'lr':            2e-4,
    'weight_decay':  1e-5,
    'es_patience':   7,     # 💡 [v2_1] 10 → 7: 더 빡빡한 조기 종료
    'es_stop_epoch': 10,    # 💡 [v2_1] 20 → 15: 더 이른 감시 시작
    'seed':          42,
    'holdout_ratio': 0.2,
}
...
# =========================================================================
class EarlyStopping:
    def __init__(self, patience=7, stop_epoch=15, verbose=True):
...
def build_xai_record(result, var_vocab_inv, vc_vocab_inv, func_vocab_inv, fold):
    ...
    return {
        ...
        'true_label': true_str, 'prediction': pred_str,
        'correct': 'O' if true_str == pred_str else 'X',
...
def main():
    ...
    print(f"[하이퍼파라미터] max_epochs={CFG['max_epochs']} | "
          f"es_patience={CFG['es_patience']} | es_stop_epoch={CFG['es_stop_epoch']}\n")
    ...
    # =========================================================================
    # Step 0: 전체에서 Final Test Set 20% 먼저 홀드아웃
    # =========================================================================
    ...
    for fold, (trainval_local, test_local) in enumerate(
        ...
        stopped_epoch = CFG['max_epochs']  # Early Stop 없으면 max_epochs
        ...
        for epoch in range(CFG['max_epochs']):
            ...
            print(f"  [Fold {fold+1} | Epoch {epoch+1:03d}/{CFG['max_epochs']}] "
            ...
            if es.early_stop:
                stopped_epoch = epoch + 1   # ← v5_4 방식: 실제 멈춘 에폭
                print(f"  [Early Stop] Fold {fold+1} → Epoch {stopped_epoch}에서 조기 종료")
                break

        if es.best_state:
            model.load_state_dict(es.best_state)
            print(f"  [복원] Best val_loss={es.val_loss_min:.4f} 시점 가중치 복원 완료")
        ...
        # Test 결과 박스에 stopped_epoch 함께 출력
        print(f"\n{'='*55}")
    ...
    # =========================================================================
    # Final Evaluation: Best Fold 모델로 홀드아웃 딱 1번 평가
    # =========================================================================
    print(f"\n{'='*55}")
    ...
    # 최종 요약에 각 Fold stopped_epoch 포함 출력
    print(f"\n{'='*55}")
