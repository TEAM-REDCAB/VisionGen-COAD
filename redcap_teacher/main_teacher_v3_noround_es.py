...
# auc 그래프 곡선화를 위해 round 수정 (build_xai 부근) - 
# es 작동하지 않앗어서 코드 수정
# 파라미터 
...
PATHS = {
    ...
    'mcat':     './mcat',
    'save_dir': './results_teacher_v3_noround_es',
}
...
CFG = {
    'n_folds':       5,
    'max_epochs':    50, # ✅ ES가 실제로 작동할 공간 확보
    'lr':            5e-5, # ✅ 권장: 4배 낮춤 → epoch당 변화량 축소
    'weight_decay':  1e-4, # ✅ 권장: 10배 강화 (L2 정규화 강화)
    'es_patience':   10, #💡 underfitting 방지
    'es_stop_epoch': 15,  # 💡 20 → 15: 더 이른 감시 시작
    'seed':          42,
...
# =========================================================================
class EarlyStopping:
    def __init__(self, patience=10, stop_epoch=15, verbose=True):
...
def build_xai_record(result, var_vocab_inv, vc_vocab_inv, func_vocab_inv, fold):
    ...
    return {
        ...
        'true_label': true_str, 'prediction': pred_str, 'probability(%)': prob_pct, 'msi_prob_raw': round(prob, 6), #(소수점 6자리, ROC용)
...
def main():
    ...
    for fold, (train_local, val_local) in enumerate(
        ...
        for epoch in range(CFG['max_epochs']):
            ...
            if es.early_stop:
                stopped_epoch = epoch + 1          # ✅ 수정 2: counter 빼지 않음
