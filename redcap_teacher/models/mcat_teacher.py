"""
models/mcat_teacher.py
Teacher 모델: 4개 모듈 조립

구조:
    PathEncoder   : WSI 패치 (N, 1536) → (N, 256)
    GenomicEncoder: 유전체  (1425, 9)  → (1425, 256)
    CoAttnFusion  : 교차 어텐션 + 퓨전  → (256,)
    ClassifierHead: 분류               → (1, 2)

Teacher-Student 증류:
    - Hinton 방식 사용 시 logits만 있으면 됨 (추가 구현 불필요)
    - 중간 표현 증류 시 attn_scores['coattn'] 사용
"""
...
from modules.coattn_fusion   import CoAttnFusion
