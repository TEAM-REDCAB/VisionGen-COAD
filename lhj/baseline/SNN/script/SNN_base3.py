...
def get_advanced_snn_features(matrix, states, vocab_size):
    ...
    # [수정] 2. Variant Classification을 '비율'로 변환하고 'MSI Score' 추가    
    idx_fs_del = vc_vocab.get('Frame_Shift_Del', 0)
    idx_fs_ins = vc_vocab.get('Frame_Shift_Ins', 0)
    idx_missense = vc_vocab.get('Missense_Mutation', 0)

    X_vc_engineered = np.zeros((num_patients, vc_vocab_size)) # 0번 패딩 포함
    msi_scores = np.zeros((num_patients, 1))
    ...
    for p in range(num_patients):
        ...
        # Variant Classification을 '비율'로 변환하고 MSI Score 계산    
        row = X_vc_raw[p]
        total_vc = np.sum(row[1:]) # 패딩 제외한 전체 카운트
        
        if total_vc > 0:
            # 비율(Proportion)로 변환
            X_vc_engineered[p, 1:] = row[1:] / total_vc
            
            # MSI Score 계산 (Indel 합산 / Missense)
            fs_total = row[idx_fs_del] + row[idx_fs_ins]
            ms_val = row[idx_missense]
            msi_scores[p] = fs_total / (ms_val + 1.0) # Zero division 방지            

    # [핵심] 여기서 0번 컬럼(Padding)을 제외하고 합칩니다.
    X_combined = np.concatenate([
        X_var_raw[:, 1:],      # Hugo
        X_vc_engineered[:, 1:], # VC 비율 (0~1 사이 값)
        X_func_raw[:, 1:],     # Function 카운트
        X_vaf_dist,            # VAF
        msi_scores             # [새로 추가] MSI Score 피처
    ], axis=1)
    
    # 각 섹션의 '수정된' 차원 정보를 반환 (SHAP 이름 매칭용)
    dims = (X_var_raw.shape[1]-1, X_vc_engineered.shape[1]-1, X_func_raw.shape[1]-1, 1)    
...
var_dim, vc_dim, func_dim, score_dim = dims
...
vaf_end = func_end + 10 # VAF는 항상 10 bin

cat_imp = {
    'Hugo_HGVSc': np.abs(shap_values[:, 0:var_end]).mean(axis=0).sum(),
    'Variant_Classification(Prop)': np.abs(shap_values[:, var_end:vc_end]).mean(axis=0).sum(),
    'Gene_Function': np.abs(shap_values[:, vc_end:func_end]).mean(axis=0).sum(),
    't_vaf': np.abs(shap_values[:, func_end:vaf_end]).mean(axis=0).sum(),
    'MSI_Score(Engineered)': np.abs(shap_values[:, vaf_end:]).mean(axis=0).sum() # 마지막 컬럼
}

# 3. 시각화
plt.figure(figsize=(10, 7))
bars = plt.barh(list(cat_imp.keys()), list(cat_imp.values()), color='mediumseagreen')
plt.title('Proposed SNN: Category Importance with MSI Score', fontsize=14)
plt.xlabel('Global Importance (Sum of SHAP)')

# MSI_Score 막대에 강조 표시 (차별화 포인트니까요!)
bars[-1].set_color('orange') 

plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(os.path.join(save_dir, 'proposed_category_importance.png'))
plt.close()
