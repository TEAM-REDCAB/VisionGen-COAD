...
def get_advanced_snn_features(matrix, states, vocab_size):
    ...
    # [수정] X_vc_raw를 제외하고 합칩니다.
    X_combined = np.concatenate([
        X_var_raw[:, 1:],   # Hugo_HGVSc
        # X_vc_raw[:, 1:],  # <--- 이 부분을 주석 처리하여 제외합니다.
        X_func_raw[:, 1:],  # Gene_Function
        X_vaf_dist          # VAF 분포
    ], axis=1)
    
    # [수정] vc_dim을 0으로 설정하거나 dims 구성에서 제외합니다.
    # 여기서는 SHAP 로직의 편의를 위해 제외된 섹션을 0으로 표기합니다.
    dims = (X_var_raw.shape[1]-1, 0, X_func_raw.shape[1]-1)
...
# [수정] 카테고리 구간 재설정 (VC가 빠진 구조 반영)
var_end = var_dim
# vc_dim은 이제 0이므로, func_end는 var_dim + func_dim이 됩니다.
func_end = var_dim + func_dim 

cat_imp = {
    'Hugo_HGVSc': np.abs(shap_values[:, 0:var_end]).mean(axis=0).sum(),
    # 'Variant_Classification': 0, # 제외되었으므로 주석 처리하거나 0으로 표시
    'Gene_Function': np.abs(shap_values[:, var_end:func_end]).mean(axis=0).sum(),
    't_vaf': np.abs(shap_values[:, func_end:]).mean(axis=0).sum()
}

# [수정] 그래프 그릴 때 존재하지 않는 키 제외
plot_data = {k: v for k, v in cat_imp.items() if v > 0}
# 3. 시각화
plt.figure(figsize=(10, 6))
plt.barh(list(plot_data.keys()), list(plot_data.values()), color='skyblue') # 색상을 바꿔서 이전 버전과 구분
plt.title('Genomic Category Importance (Excluding Variant Classification)')
plt.xlabel('Global Importance (Sum of SHAP)')
plt.gca().invert_yaxis()
plt.savefig(os.path.join(save_dir, 'category_importance_no_vc.png'), bbox_inches='tight')
