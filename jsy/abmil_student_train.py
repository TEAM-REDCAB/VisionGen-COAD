...
from utils.h5dataset_full import (
    H5Dataset,
)  # kd_path를 처리할 수 있게 수정된 데이터셋 사용
...
MODEL_PATH = os.path.join(
    RESULTS_PATH, "saved_models_abmil_kd"
)  # KD 전용 폴더로 분리 권장
...
for fold_idx in range(5):
    ...
    for epoch in range(num_epochs):
        ...
        if val_auroc > best_val_auroc:
            ...
            print(
                f"🔥 Fold {fold_idx} 최고 성능 갱신! 모델 저장됨 (AUROC: {val_auroc:.4f}), (AUPRC: {val_auprc:.4f}), (Best Threshold: {val_thresh:.4f})"
            )

    fold_results.append(best_val_auroc)
    print(
        f"Fold {fold_idx} 종료. AUROC: {best_val_auroc:.4f}, AUPRC: {best_val_auprc:.4f}, 최적 Threshold: {best_thresh:.4f}"
    )
