...
def get_feats_path():
    if DATA_PATH == "gigapath":
        ...
        return (
            "/home/team1/data/cptac_processed/20.0x_256px_0px_overlap/features_gigapath"
        )
...
def get_label_path(patients_path):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(
        skf.split(df_train_val, df_train_val["msi"])
    ):
    ...
    def __getitem__(self, idx):
        ...
        if not file_names:
            raise FileNotFoundError(
                f"환자 {patient_id}에 대한 .h5 파일을 찾을 수 없습니다."
            )
        ...
        genomic_features = torch.tensor(
            self.genomic_matrix[npy_idx], dtype=torch.float32
        )
