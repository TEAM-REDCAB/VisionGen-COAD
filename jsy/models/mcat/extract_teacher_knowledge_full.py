...
def export_knowledge():
    ...
    model_path = os.path.join(RESULT_PATH, 'best_model_full.pt')
    
    if not os.path.exists(model_path):
        print("⚠️모델 파일이 없습니다. 건너뜁니다.")
        return
        
    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
    if 'model_state_dict' in checkpoint:
        model.load_state_dict(checkpoint['model_state_dict'])
        print("체크포인트 딕셔너리에서 'model_state_dict' 로드 완료.")
    else:
    ...
    save_file = os.path.join(KNOWLEDGE_SAVE_DIR, 'knowledge_full_train.pkl')
    ...
    avg_omic_file = os.path.join(KNOWLEDGE_SAVE_DIR, 'avg_omic_full.pt')
    torch.save(avg_omic, avg_omic_file)
        
    print(f"✅ 지식 저장 완료: {save_file} ({len(fold_knowledge)}명)")
