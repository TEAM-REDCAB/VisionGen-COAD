...
def test_and_visualize(threshold=0.35):
    ...
    for fold in range(5):
        ...
        with torch.no_grad():
            pbar = tqdm(
                test_loader, desc=f"[Fold {fold}]", leave=False, dynamic_ncols=True
            )
    ...
    report = classification_report(
        all_labels, preds, target_names=["MSS (0)", "MSI-H (1)"], zero_division=0
    )
    ...
    #     color="blue",
    #     s=200,
    #     label=f"Best Thresh ({ensemble_best_thresh:.4f})",
    #     zorder=5,
    # )
    ...
    ax2.plot(
        recall, precision, color="blue", lw=2, label=f"Ensemble PR (AUC = {auprc:.4f})"
    )
    ...
    plt.savefig(
        os.path.join(TEST_PATH, "confusion_matrix_MCAT_kd_ensemble.png"), dpi=300
    )
    plt.show()
    ...
    plt.axvline(
        ...
        linestyle="--",
        lw=2,
        label=f"Threshold ({ensemble_best_thresh:.4f})",
    )
    ...
    results_df.to_csv(
        os.path.join(TEST_PATH, "test_results_MCAT_kd_ensemble.csv"), index=False
    )
