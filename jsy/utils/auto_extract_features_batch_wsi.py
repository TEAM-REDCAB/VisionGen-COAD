...
def get_path(path):
    ...
    return os.path.join(current_dir, 'working', path)

# --- [설정 구간] ---
PATCH_ENCODER = 'gigapath'    # 패치 인코딩 모델(uni_v2, gigapath 등)                 
MANIFEST_PATH = get_path('common_manifest.txt')
GDC_CLIENT_PATH = get_path('gdc-client')
WSI_TEMP_DIR = get_path('temp_wsis')          # UUID 폴더들이 담길 곳
WSI_FLAT_DIR = get_path('flat_wsis')          # 이름이 바뀐 SVS 파일이 모일 곳
RESULT_DIR = get_path('trident_processed')    
BATCH_SIZE = 1
...
df['slide_id'] = df['filename'].apply(lambda x: x.split('.')[0] + '.svs')
new_df = df[df['slide_id'].apply(lambda x: not os.path.exists(os.path.join(WSI_FLAT_DIR, x)))]
total_slides = len(df)
...
for i in range(0, total_slides, BATCH_SIZE):
    ...
    try:
        ...
        trident_cmd = (
            f"python ./run_batch_of_slides.py --task all "
