...
def kd_loss_fn(
    s_logits, s_path_bag, s_attn,
    t_logits, t_path_bag, t_attn,
    labels, task_criterion, epoch,
    alpha, beta, gamma, T=2.0
):
    """
    지식 증류 복합 손실 함수
    ...
    Args:
        ...
        T                  : temperature (logit softening)
    """

    # ── 1. Task Loss ────────────────────────────────────────────────────────
    task_loss = task_criterion(s_logits, labels)

    # ── 2. Logit Distillation : temperature-scaled BCE ──────────────────────
    # T²를 손실에 곱하면 큰 배율이 생기므로 alpha를 낮게 가져감 (0.3)
    s_prob = torch.sigmoid(s_logits / T)
    t_prob = torch.sigmoid(t_logits / T)
    if t_prob.shape != s_prob.shape:
        t_prob = t_prob.view_as(s_prob)
    logit_loss = F.binary_cross_entropy(s_prob, t_prob.detach()) * (T ** 2)
    ...
    task_weight = 0.0 if epoch < 10 else 1.0 - alpha
    total_loss = (task_weight * task_loss) + (alpha * logit_loss) + (beta * attn_loss) + (gamma * path_loss)
    return total_loss, task_loss, logit_loss, attn_loss, path_loss


def train_binary(epoch, model, loader, optimizer, loss_fn, gc=16, alpha=0.05, beta=0.5, gamma=0.05):
    ...
    sum_task = sum_logit = sum_attn = sum_path = 0.
    latent_grad_norms = []
    ...
    for batch_idx, (features, coords, labels, t_logits, t_path_bag, t_attn) in enumerate(pbar):
        ...
        if (batch_idx + 1) % gc == 0 or (batch_idx + 1) == len(loader):
            optimizer.step()
            if hasattr(model, 'latent_queries') and model.latent_queries.grad is not None:
    ...
    print(
        ...
        f'Task={0.0 if epoch < 10 else 1.0 - alpha:.4f} | Logit(alpha)={alpha} | Attn(beta)={beta} | Path(gamma)={gamma}'
    )
    print(
